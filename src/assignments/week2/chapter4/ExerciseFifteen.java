package assignments.week2.chapter4;

/*
--Exercise 15--
Write a method called getGrade that accepts an integer representing a student’s grade in a course and returns
that student’s numerical course grade. The grade can be between 0.0 (failing) and 4.0 (perfect).

Assume that scores are in the range of 0 to 100 and that grades are based on the following scale:

Score	Grade
-------------
<60     0.0
60–62	0.7
63      0.8
64	    0.9
65	    1.0
...     ...
92      3.7
93	    3.8
94	    3.9
>= 95	4.0

For an added challenge, make your method throw an IllegalArgumentException
if the user passes a grade lower than 0 or higher than 100.

*/

public class ExerciseFifteen {
    public static void main(String[] args) {
        double grade = getGrade(95);
        System.out.println(grade);
        double grade1 = getGrade(100);
        System.out.println(grade1);
        double grade2 = getGrade(0);
        System.out.println(grade2);
        double grade4 = getGrade(92);
        System.out.println(grade4);
        double grade3 = getGrade(101);
        System.out.println(grade3);
    }

    public static double getGrade(int score) {
        if (score < 0 || score > 100) {
            throw new IllegalArgumentException("Invalid score");
        } else if (score < 60) {
            return 0;
        } else if (score < 63) {
            return 0.7;
        } else if (score >= 95) {
            return 4.0;
        } else {
            return 0.7 + (score - 62) / 10.0;
        }
    }
}
