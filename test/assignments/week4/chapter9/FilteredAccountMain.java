...
public class FilteredAccountMain {
    public static void main(String[] args) {
        Client client = new Client("John Doe");
        FilteredAccount account = new FilteredAccount(client);
        account.process(new Transaction(0));
        account.process(new Transaction(100));
        account.process(new Transaction(0));
        account.process(new Transaction(0));
        account.process(new Transaction(-10));
        account.process(new Transaction(20));
        account.process(new Transaction(-60));
        System.out.println(account.percentFiltered());
    }
}
